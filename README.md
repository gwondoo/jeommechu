...
회사 좌표가 20m 이내로 같고 검색 반경도 같은 Discord 서버들은 주변 식당 검색 캐시를 공유합니다. 식권대장과 추천 이력은 계속 서버별로 분리됩니다.

종류를 지정하지 않은 주변 추천은 반경 안에서 한식, 중식, 일식, 양식, 아시아음식, 분식, 치킨, 피자와 기타 음식점을 각각 최대 45곳 조회합니다. 그룹별 결과를 SQLite에 따로 저장하고, 추천할 때 합쳐 카카오 장소 ID로 중복을 제거합니다. 모든 검색에 카카오 음식점 그룹(`FD6`)을 강제해 별도 카페 그룹(`CE7`)을 제외하며, `FD6` 안에 포함된 술집, 칵테일바, 호프, 요리주점, 이자카야, 와인바와 포장마차도 점심 후보에서 제외합니다. 각 그룹 결과는 6시간 동안 재사용합니다.
